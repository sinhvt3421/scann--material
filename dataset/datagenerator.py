...
class DataIterator(object):
    ...
    def __init__(self, type, batch_size=32, nsample=0,
                 ...
                 intensive=True, converter=True, use_ring=False, use_bonds=False,
    ...
    def iterator(self):
        while 1:
            ...
            # future = [pool.submit(self._get_batches_of_transformed_samples,
            #                         self.indices[current_index: min(current_index +
            #                                     self.batch_size,self.n)],) for current_index in range(0,self.num_batch,self.batch_size)]
            # for f in as_completed(future):
            #     minibatch = f.result()
            #     yield minibatch

            for current_index in range(0, self.num_batch, self.batch_size):
                minibatch = self._get_batches_of_transformed_samples(self.indices[current_index: min(current_index +
